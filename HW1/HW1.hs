...
leftmost :: Tree -> Int
leftmost (Leaf x) = x
leftmost (Node y b c) = leftmost b
...
rightmost :: Tree -> Int
rightmost (Leaf x) = x
rightmost (Node y b c) = rightmost c
...
--
maxInt :: Tree -> Int
maxInt (Node x a b) = max x (max (maxInt a) (maxInt b))
maxInt (Leaf x) = x
...
--
minInt :: Tree -> Int
minInt (Node x a b) = min x (min (minInt a) (minInt b))
minInt (Leaf x) = x
...
--
sumInts :: Tree -> Int
sumInts (Node x a b) = x + sumInts a + sumInts b
sumInts (Leaf x) = x
...
preorder :: Tree -> [Int]
preorder (Leaf x) = [x]
preorder (Node y a b) = y : preorder a ++ preorder b
...
inorder :: Tree -> [Int]
inorder (Leaf x) = [x]
inorder (Node y a b) = inorder a ++ y : inorder b
...
--   
isBST :: Tree -> Bool
isBST (Node y a b) = (maxInt a < y) && (minInt b > y) && isBST a && isBST b
isBST (Leaf x) = True
...
--   
inBST :: Int -> Tree -> Bool 
inBST n (Node y a b) | n > y = inBST n b || n == y
inBST n (Node y a b) | n <= y = inBST n a || n == y
inBST n (Leaf x) = n == x
