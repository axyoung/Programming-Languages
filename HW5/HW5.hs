...
checkExpr vars e = case e of
                        Ref v -> v `elem` vars
                        Add l r -> checkExpr vars l && checkExpr vars r
                        Mul l r -> checkExpr vars l && checkExpr vars r
                        Lit i -> True
...
checkCmd map vars (Pen m) = True
checkCmd map vars (Move l r) = checkExpr vars l && checkExpr vars r
checkCmd map vars (Call m a) = case lookup m map of
  Just i -> i == length a && all (checkExpr vars) a
  Nothing -> False
checkCmd map vars (For v l r b) = checkExpr vars l && checkExpr vars r && checkBlock map (v:vars) b
...
checkBlock [] vars block = all (checkCmd [] vars) block
checkBlock map vars block = all (checkCmd map vars) block
...
checkDef map (Define m p b) = checkBlock map p b
...
--
expr :: Env -> Expr -> Int
expr ((n, i):ns) (Ref v) = if v == n then i else expr ns (Ref v)
expr e (Lit i) = i
expr e (Add a b) = expr e a + expr e b
expr e (Mul a b) = expr e a * expr e b
...
cmd defs env state@(pen,pos) c = case c of
    ...
    Move xExp yExp -> if pen == Down
      then ((pen, (expr env xExp, expr env yExp)), [(pos, (expr env xExp, expr env yExp))])
      else ((pen, (expr env xExp, expr env yExp)), [])

    Call macro args -> case lookup macro defs of
      Just (ps, b) -> block defs (setAll ps (exprs env args) env) state b
      Nothing -> (state, [])
...
exprs :: Env -> [Expr] -> [Int]
exprs env (e:es) = expr env e : exprs env es
