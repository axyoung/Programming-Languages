...
encodeList :: [a] -> Tree a
encodeList [] = End
encodeList (h:t) = Node h (encodeList []) (encodeList t)
...
valueAt [] End = Nothing
valueAt (R:t) (Node v l r) = valueAt t r
valueAt (L:t) (Node v l r) = valueAt t l
valueAt [] (Node v l r) = Just v
...
pathTo n (Node v l r)
  | n == v = Just []
  | otherwise = case (pathTo n l, pathTo n r) of
                (Just p, Nothing) -> Just (L:p)
                (Nothing, Just p) -> Just (R:p)
                _ -> Nothing
