...
--
boxBody :: Block
boxBody = [Pen Up, Move (Ref "x") (Ref "y"), Pen Down, Move (Add (Ref "x") (Ref "w")) (Ref "y"), 
           Move (Add (Ref "x") (Ref "w")) (Add (Ref "y") (Ref "h")), Move (Ref "x") (Add (Ref "y") (Ref "h")),
           Move (Ref "x") (Ref "y")]
...
prettyCmd (Call m a) = m ++ "(" ++ prettyArgs a ++ ")"
prettyCmd (For i a b m) = "for " ++ prettyExpr (Ref i) ++ " = " ++ prettyExpr a ++ " to " ++ prettyExpr b ++ " " ++ prettyBlock m

prettyArgs :: Args -> String
prettyArgs [] = "{}"
prettyArgs x = prettyExprs x
  where
    prettyExprs = intercalate ", " . map prettyExpr
